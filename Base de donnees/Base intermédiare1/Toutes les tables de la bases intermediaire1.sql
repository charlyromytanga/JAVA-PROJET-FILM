...
-- ------------------------------------------------------------------------------
DROP DATABASE IF EXISTS vente_films_en_ligne1;
CREATE DATABASE vente_films_en_ligne1;
USE vente_films_en_ligne1;
...
BEGIN
    ...
    DECLARE userId INT;
    DECLARE characters VARCHAR(62) DEFAULT 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'; -- Définir la variable characters

    -- Supprimer la table Liste_Noms si elle existe
    DROP TABLE IF EXISTS Liste_Noms;
    -- Créer la table Liste_Noms
    CREATE TEMPORARY TABLE Liste_Noms (nom VARCHAR(50));
    -- Insérer des données dans la table Liste_Noms (50 noms du calendrier)
    INSERT INTO Liste_Noms (nom) VALUES 
        ('Smith'), ('Johnsons'), ('Williams'), ('Brown'), ('Jones'),
    ...
    -- Supprimer la table Liste_Prenoms si elle existe
    DROP TABLE IF EXISTS Liste_Prenoms;
    -- Créer la table Liste_Prenoms
    CREATE TEMPORARY TABLE Liste_Prenoms (prenom VARCHAR(50));
    -- Insérer des données dans la table Liste_Prenoms (50 prénoms du calendrier)
    INSERT INTO Liste_Prenoms (prenom) VALUES 
        ('James'), ('John'), ('Robert'), ('Michael'), ('William'),
        ('David'), ('Richard'), ('Joseph'), ('Charles'), ('Thomas'),
        ('Daniel'), ('Matthew'), ('Anthony'), ('Mark'), ('Donald'),
        ('Steven'), ('Paul'), ('Andrew'), ('Kenneth'), ('Joshua'),
        ('Kevin'), ('Brian'), ('George'), ('Edward'), ('Ronald'),
        ('Timothy'), ('Jason'), ('Jeffrey'), ('Ryan'), ('Jacob'),
        ('Gary'), ('Nicholas'), ('Eric'), ('Stephen'), ('Jonathan'),
        ('Larry'), ('Justin'), ('Scott'), ('Brandon'), ('Frank'),
        ('Benjamin'), ('Gregory'), ('Samuel'), ('Raymond'), ('Patrick'),
        ('Alexander'), ('Jack'), ('Dennis'), ('Jerry'), ('Tyler');

    WHILE i < 500 DO
        -- Sélectionner un prénom aléatoire
        SET @prenom := (SELECT nom FROM (SELECT nom FROM Liste_Noms ORDER BY RAND() LIMIT 1) AS temp);

        -- Sélectionner un nom aléatoire
        SET @nom := (SELECT prenom FROM (SELECT prenom FROM Liste_Prenoms ORDER BY RAND() LIMIT 1) AS temp);
